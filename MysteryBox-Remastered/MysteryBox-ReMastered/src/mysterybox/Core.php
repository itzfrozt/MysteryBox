 ...
 * Created by TheAz928
 * Edited by ItzFrozt for 3.0.0 api
 *
...
class Core extends PluginBase{
	...
	public function onLoad(){
		...
		$this->getLogger()->info(TF::colorize("&2Loading Mystery Boxes from config..."));
		
		$this->saveDefaultConfig();
		
		if($this->getConfig()->get("version", null) !== self::config_ver){
			$this->getLogger()->info(TF::colorize("&2Config version seems to be incompatible with this version, resetting..."));
			$this->getLogger()->info(TF::colorize("&2If you've a config backup, update your backup with this version before mounting"));
		...
		foreach($this->getConfig()->get("boxes", []) as $key => $data){
			try{
				$this->getLogger()->info(TF::colorize("&2Loading Mystery Box: ".($data["name"] ?? "&2UNDEFINED")));
				$box = new MysteryBox($data);
				$this->loadMysteryBox($box);
			}catch(\Exception $e){
                                $this->getLogger()->logException($e);
				$this->getLogger()->info(TF::colorize("&2Unable to load Mystery Box &f#".$key));
			}
		}
	}
	...
	public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args) : bool{
		switch($args[0] ?? null){
			default: {
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox &e[add | remove | set | create]"));
				break;
			}
			case "add": {
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox add &c[name] [box id] [amount]"));
					break;
				}
				if(isset($args[2]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox add &a[name] &c[box id] [amount]"));
					break;
				}
				if(isset($args[3]) == false or is_numeric($args[3]) == false or $args[3] < 1){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox add &a[name] [box id] &c[amount]"));
					break;
				}
				...
				if($box == null){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2Mystery Box with ID &e{$id}&7 doesn't exist"));
					break;
				}
				
				$this->addMysteryKey($player, $id, $int);
				
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2Added &e{$id}&7 key × &b{$int}&7 to &f{$args[1]}'s &7account"));
				break;
			}
			case "remove": {
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox remove &c[name] [box id] [amount]"));
					break;
				}
				if(isset($args[2]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox remove &a[name] &c[box id] [amount]"));
					break;
				}
				if(isset($args[3]) == false or is_numeric($args[3]) == false or $args[3] < 1){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox remove &a[name] [box id] &c[amount]"));
					break;
				}
				...
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2Removed &e{$id}&7 key × &b{$int}&7 from &f{$args[1]}'s &7account"));
				break;
			}
			case "set": {
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox set &c[name] [box id] [amount]"));
					break;
				}
				if(isset($args[2]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox set &a[name] &c[box id] [amount]"));
					break;
				}
				if(isset($args[3]) == false or is_numeric($args[3]) == false or $args[3] < 1){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox set &a[name] [box id] &c[amount]"));
					break;
				}
				...
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2Set &e{$id}&7 key × &b{$int}&7 to &f{$args[1]}'s &7account"));
				break;
			}
			case "create": {
				if($sender instanceof Player == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox create can only be ran in-game!"));
					break;
				}
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2/mysterybox create &c[box id]"));
					break;
				}
				if($this->getMysteryBox($args[1]) == null){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2Invalid Mystery Box ID"));
					break;
				}
				
				$this->getEventListener()->queueBoxCreation($sender, $args[1]);
				
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&2Tap a block to turn it into a Mystery Box"));
				break;
			}
			case "reload": {
				$this->onDisable();
