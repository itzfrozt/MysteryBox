...
class EventListener implements Listener{
	...
	public function onInteract(PlayerInteractEvent $event) : void{
		...
		if(($id = $this->sessions[$player->getId()] ?? null) !== null){
			...
			$box = Block::get(Block::END_PORTAL_FRAME, $m[$player->getDirection()]);
		...
		if($tile instanceof MysteryTile){
			...
			if($tile->in_use){
				$player->sendTip(TF::colorize($this->getCore()->getConfig()->get("prefix")." &l&bWARN &r&7Crate is already in use!"));
				return;
			}
			...
			if($box->canOpen($player)){
				...
				$player->sendTip(TF::colorize($this->getCore()->getConfig()->get("prefix")." &l&bWARN &r&7You don't have any keys to open &2{$box->getName()}"));
			}
		}
	}
